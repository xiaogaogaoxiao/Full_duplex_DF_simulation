...
for s = 1:length(Eb_N0_all) % SNR Loop
    ...
    h_rr = 1/sqrt(2)*((randn(Tx_number,1)+j*randn(Tx_number,1)))*sqrt(h_rr_gain_Linear)*0; % Rayleigh
    h_sd = 1/sqrt(2)*((randn(Tx_number,1)+j*randn(Tx_number,1)))*sqrt(h_sd_gain_Linear)*0; % Rayleigh
    ...
    parfor Tx = 1:Tx_number % Transmission Loop
        ...
        %% Noise (S-R)
        sigma = sqrt(10^(-Eb_N0/10));
        n_r = sqrt(sigma/2)*(randn(L+tau,1) + j*randn(L+tau,1));
        %% Received Signal (S-R)
        y_r = H_1 * x_s + n_r;
        ...
        %% Noise (R-D)
        sigma = sqrt(10^(-Eb_N0/10));
        n_d = sqrt(sigma/2)*(randn(L+tau,1) + j*randn(L+tau,1));
        %% Received Signal (R-D)
